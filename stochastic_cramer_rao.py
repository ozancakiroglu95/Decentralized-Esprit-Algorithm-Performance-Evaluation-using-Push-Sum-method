def cramer_rao(A, signal, angles, locations):
    ...
    der_V = [np.zeros((12,6), dtype=complex),np.zeros((12,6),dtype=complex),np.zeros((12,6),dtype=complex)]
    V = [np.zeros((12,6), dtype=complex),np.zeros((12,6),dtype=complex),np.zeros((12,6),dtype=complex)]

    h = np.ones((6,1))
    #h = np.atleast_2d([1,0,0,0,0,0]).T
    ...
    D_theta = np.squeeze(np.array([der_V[0].dot(h),der_V[1].dot(h),der_V[2].dot(h)])).T
    D_xi = []
    D_zheta = []
    e_k = np.zeros((6,1))
